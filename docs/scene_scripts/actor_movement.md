...
If bit `$02` of `move_dest_flags` is set, actors will try to detect solid actors (with bit `$01` of `solidity_flags`
set) during movent updates. If one is found in front of where the actor is facing, the calculated movement angle is
offset by 67.5 degrees clockwise.
