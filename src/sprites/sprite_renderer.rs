...
use super::sprite_assembly::SpriteAssemblyTileFlags;
...
pub fn render_sprite(surface: &mut Surface, pixel_source: &mut Bitmap, source_value: u8, sprite: &Sprite, frame: usize, x: i32, y: i32, palette_offset: usize) {
    ...
    for tile in frame.tiles.iter().rev() {
        if tile.flags.contains(SpriteAssemblyTileFlags::UNUSED) {
            continue;
        }
        ...
        if tile.flags.contains(SpriteAssemblyTileFlags::FLIP_X) {
            flags |= BitmapBlitFlags::FLIP_X;
        }
        if tile.flags.contains(SpriteAssemblyTileFlags::FLIP_Y) {
